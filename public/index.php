  ...
  if (strpos($_SERVER["REQUEST_URI"], "/api/list") === 0) {
    ...
    if (isset($_GET["cityvalue"]) && $_GET["cityvalue"] != "") {
      if (is_int($_GET["cityvalue"])) {
        sendResponse(200, "Cities successfully requested, using postcode!",  array_search($_GET["cityvalue"], json_decode(file_get_contents("../common/all_cities.json", true), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)));
