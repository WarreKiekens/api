...
function filtering_cities() {
  ...
  if (in_array($_GET["like"], array("t","f"))) {
    ...
    $query = "SELECT id,naam,postcode,isactief,isnew,emailadres,(select count(influencerid) from InfluencerStad where stadid = stad.id) as influencercount,nietwinnaarreward FROM stad WHERE {$_GET['where']} = $1 ORDER BY id";
    ...
    $query = "SELECT id,naam,postcode,isactief,isnew,emailadres,(select count(influencerid) from InfluencerStad where stadid = stad.id) as influencercount,nietwinnaarreward FROM stad WHERE position($1 in {$_GET['where']}) > 0 ORDER BY id";
