<?php
  ...
  function get_details_rewards($id){
    ...
    $query = "select * from reward where id in (select rewardid from influencerreward where influencerid = $1);";
    $data1 = fetch_query_params($query, array($id));

    // all other rewards
    $query = "select * from reward where id not in (select id from reward where id in (select rewardid from influencerreward where influencerid = $1));";
